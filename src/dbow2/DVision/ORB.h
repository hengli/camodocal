 ...
 * Author: Lionel Heng
 * Date: February 2013
 * Description: implementation of ORB (Oriented FAST and
 *   Rotated BRIEF) descriptor by
...
public:
  ...
  /**
   * Creates the ORB a priori data for descriptors of nbits length
   * @param nbits descriptor length in bits
   * @param patch_size 
   * @param type type of pairs to generate
   */
  ORB(int nbits = 256, int patch_size = 31, Type type = RANDOM_CLOSE);
  virtual ~ORB();
  ...
  /**
   * Returns the ORB descriptors of the given keypoints in the given image
   * @param image
   * @param points
   * @param descriptors 
   ...
   * @note this function is similar to ORB::compute
   */
   ...
   *   grayscale if needed and smoothed. If not, it is assumed the image has
   *   been treated by the user
   * @note this function is similar to ORB::operator()
   */ 
