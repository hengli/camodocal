...
CameraRigBA::buildVocTree(void)
{
    ...
    for (size_t segmentId = 0; segmentId < m_graph.frameSetSegments().size(); ++segmentId)
    {
        FrameSetSegment& segment = m_graph.frameSetSegment(segmentId);

        if (m_verbose)
        {
            std::cout << "# INFO: Computing bags-of-words for frame set segment "
                      << segmentId + 1 << "/" << m_graph.frameSetSegments().size()
                      << std::endl;
        }
        ...
        for (size_t frameSetId = 0; frameSetId < segment.size(); ++frameSetId)
        {
            ...
            {
                FramePtr& frame = frameSet->frames().at(frameId);
                ...
                FrameTag fid;
                fid.frameSetSegmentId = segmentId;
                fid.frameSetId = frameSetId;
                fid.frameId = frameId;
                m_v2fLUT.push_back(fid);
                                       ...
                                       double reprojErrorThresh)
{
    ...
    m_db.query(frameToBOW(frameQuery), ret, k_nearestImageMatches);
    ...
    {
        FrameTag frameTag = m_v2fLUT.at(ret.at(i).Id);
        ...
        std::vector<cv::DMatch> matches = matchFeatures(frameQuery->features2D(), frame->features2D());
...
CameraRigBA::visualize(const std::string& overlayPrefix, int type)
{
    ...
    for (size_t i = 0; i < m_cameras.size(); ++i)
    {
        ...
        std::vector<PoseEPtr> cameraPoses;
        boost::unordered_set<Point3DFeature*> scenePointSet;
        ...
        for (size_t j = 0; j < m_graph.frameSetSegments().size(); ++j)
        {
            ...
            for (size_t k = 0; k < segment.size(); ++k)
            {
                ...
                if (i == 0)
                {
                    odometryVec.push_back(frameSet->systemPose());
                }
        ...
        else
        {
            ...
            {
                PoseEPtr& cameraPose = cameraPoses.at(j);
